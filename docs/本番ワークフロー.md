...
本番は Cloudflare に置く。毎晩 04:30 JST に Cloudflare Workflows の `schedules` が 1 インスタンスを起こす。夜業は **二段** にする。(1) GitHub / npm へ機械的にアクセスし、生データを D1 に保存する。(2) 保存済み JSON だけを見て号を書き、D1 に保存してからメールする。生成は AI Gateway 経由の xAI `grok-4.6`。失敗したらテンプレ。LLM に GitHub を触らせない。空の朝は成功である。永続化は D1。GitHub は App の user-to-server。
...
- マルチテナント SaaS にしない。運用の主語はまだ一人。スキーマだけユーザと OSS を多対多にし、上流 Releases をユーザ人数倍に取らない。
...
5. 収集と生成を分ける。収集は GitHub だけ。生成は D1 の JSON だけを見る。LLM 失敗時は `renderTemplateEdition` で号が閉じる。
...
- 複数タイムゾーン、1 日複数号、課金付きのマルチテナント SaaS。
...
| 本文 | 保存済み JSON から生成。主経路は LLM。失敗時はテンプレ |
...
| LLM | **AI Gateway → grok**（`grok-4.6`）。Workers AI は主経路にしない。入力は D1 の生データだけ |
...
flowchart TB
  ...
  subgraph wf [NightlyEdition Workflow]
    ...
    S4["4. Ingest: npm map + Releases + Advisories → D1"]
    S5["5. Judge: print_new / hold / resolve（機械）"]
    S6["6. Generate: 保存済み JSON → 号（LLM / 失敗時テンプレ）"]
  ...
  subgraph store [D1]
    USERS[users / oauth_credentials]
    BEAT[user_repos]
    PKG[packages / user_packages]
    RAW[package_releases / package_advisories]
    CUR[package_cursors]
...
先頭 step `claim-run` の戻りは `{ action: "skip" | "full" | "generate_only" | "deliver_only" }`。UNIQUE 衝突は **throw しない**（step 失敗にすると at-least-once の cron が claim で死ぬ）。
...
async function claimPipelineRun(
  ...
  editionDate: string,
  instanceId: string,
): Promise<{ action: "skip" | "full" | "generate_only" | "deliver_only" }> {
  try {
...
| `action` | 意味 | `run()` |
| --- | --- | --- |
| `full` | この instance が収集からやる | `reset-night` → refresh → Beat → ingest → judge → generate → deliver |
| `skip` | `delivered` / `empty_ok`、または他が実行中 | 即 return。GitHub も LLM もメールもしない |
| `generate_only` | 収集済み、号は未作成 | **GitHub を飛ばし** raw を読んで generate → deliver |
| `deliver_only` | 号は `composed`、未配信 | **refresh / Beat / ingest / generate を飛ばし** `deliver` だけ |

`pipeline_runs.status` の遷移は **`running` → `ingested` → `composed` → `delivered` | `empty_ok`**。`ingested` は raw + `night_decisions` まで。`composed` は号の body。`empty_ok` は空号の **配信成功後だけ**。`complete` は使わない。
...
export class NightlyEdition extends WorkflowEntrypoint<Env, { editionDate?: string }> {
  async run(event: WorkflowEvent<{ editionDate?: string }>, step: WorkflowStep) {
    ...
    try {
      ...
      if (claimed.action === "generate_only") {
        await this.generateAndDeliver(editionDate);
...
sequenceDiagram
  ...
  Note over W,D1: 一段目 収集（GitHub だけ。LLM なし）
  ...
  loop ライブラリ チャンク
    ...
    W->>D1: raw_investigations（リリース JSON。全文リポジトリは置かない）
  end
  W->>D1: judgments + stories + package_cursors
  Note over W,GW: 二段目 生成（GitHub に戻らない）
  W->>D1: print_new の raw JSON
  W->>GW: grok-4.6（事実 JSON のみ）
  alt LLM 成功
    W->>D1: editions.body（LLM）
  else LLM 失敗 / 未設定
...
**heuristic CopyFact**（テンプレ経路。LLM 生成の入力は raw JSON）
...
任意 LLM は生成段だけ。収集の heuristic はテンプレ用の逃げ。
...
夜業を同じ step に混ぜない。GitHub の失敗と LLM の失敗を切り分ける。

| 段 | 外向き | D1 に残す | 失敗したら |
| --- | --- | --- | --- |
| 収集 | GitHub GraphQL/REST、npm | `beat_repos`、`snapshots`、`raw_investigations`（リリース / GHSA の JSON。本文は 32 KiB で切る。行 2 MB 制限） | step retry。LLM は呼ばない |
| 判断 | なし（機械） | `night_decisions`、cursors、Story | 収集済みを見て hold / print。再収集しない |
| 生成 | **AI Gateway → grok だけ** | `editions` / `edition_items` | 同じ JSON でテンプレ。GitHub に戻らない |

`deliver_only` に加え、収集済み・未生成なら `generate_only`（refresh/Beat なし、raw を読んで生成→配信）。収集済みの再生成は GitHub rate limit を食わない。

LLM に Web Search / GitHub ツールを付けない。知識カットオフ（grok-4.6 は 2026-02-01）があっても、入力 JSON 以外の事実を足さない。出典 URL は JSON にあるものだけ。

#### 5. CopyFact → 号

入力は D1 の `raw_investigations` と `night_decisions`（print + 今夜初見の human）。GitHub には戻らない。0 print なら「新ニュースなし。」をテンプレで書いて配信する（LLM を呼ばない）。

1. **主: AI Gateway 経由の grok-4.6。** print 対象の生 JSON（tag、publishedAt、body 先頭、出典 URL）を渡し、headline / lead / 出典だけを返す。事実にない数字を足したら破棄してテンプレ。
2. **失敗時: `renderTemplateEdition`（`src/copy.ts`）。** heuristic の `CopyFact`。号は止まらない。

`factsForEdition` はテンプレ経路用。LLM 経路は raw JSON を直接見る。古い major / パッチの human hold 2 日目は silent。枠外の 9 本目は紙面に出さず、翌日また候補。
...
- `XAI_API_KEY` は **required に入れない**（BYOK / Unified のときは Gateway 側）。未設定なら生成はテンプレ。
- `DELIVER` / `LLM_GENERATE` は secret ではなく `vars`。`0` なら生成はテンプレのみ。
...
スパイクの `src/schema.sql` は連続テスト用に残す。本番 0001 は drift 表を複製せず、共有 OSS カタログから始める（[収集.md](収集.md)）。`wrangler d1 migrations apply`（[D1 migrations](https://developers.cloudflare.com/d1/reference/migrations/)、2026-06-08）。

```sql
-- 0001_catalog.sql は migrations/ が正本。users × packages が多対多。
```

```sql
-- 後続（Story / 号 / cursor）。カタログの上に足す。
...
CREATE TABLE users (
  id TEXT PRIMARY KEY,              -- github login（v1 allowlist と同じ）
...
| テーブル | 1 日 | 1 年 |
| --- | --- | --- |
| user_repos | ユーザあたり ~10–100 | 増えない（特定済み集合） |
| packages / user_packages | unique OSS 数十 | 共有なのでユーザ人数に比例しない |
| package_releases | パッケージあたり tag 数十 | 数十 MB 未満 |
...
### 3. 収集と生成を分ける vs 同じ step で LLM に GitHub を触らせる

| | 二段（収集 → 保存 → 生成） | 一段（LLM がツールで GitHub） | テンプレのみ |
| --- | --- | --- | --- |
| GitHub 失敗 | 生成しない。再収集だけ | 生成と混ざる | 収集ができれば号は出る |
| LLM 失敗 | 同じ JSON でテンプレ / 再生成 | 最初から | 関係ない |
| 発明 | JSON 以外を禁止できる | ツール結果と記憶が混ざる | しない |
| 再実行 | `generate_only` | rate limit を再消費 | 無料 |

検証（`docs/紙面生成.md`）: 事実 JSON が切ってあればテンプレで届く。LLM の差は文章。**不安定なのは取材と hold。** だから収集と判断は機械、文章だけ LLM。

**採用: 二段。** 収集は機械。判断は機械。生成は LLM、失敗時テンプレ。

xAI 料金（[Models](https://docs.x.ai/docs/models)）: `grok-4.6` は prompt < 200k で Input $2 / Cached $0.50 / Output $6 per 1M。知識カットオフ 2026-02-01。**モデルの記憶で事実を足さない。** 入力は D1 の JSON のみ。8 項 × 入出力 4k token でも 1 夜 $0.01 未満。
...
### 6. LLM 経路: xAI 直接 vs Workers AI vs AI Gateway → grok

AI Gateway は推論エンジンではない。**プロキシ**である（ログ、キャッシュ、レート制限、フォールバック、Unified Billing）。Grok は公式に載っている。

- エンドポイント: `https://gateway.ai.cloudflare.com/v1/{account_id}/{gateway_id}/grok`（[xAI provider](https://developers.cloudflare.com/ai-gateway/usage/providers/grok/)、2026-04-20）
- `https://api.x.ai/v1` をこの URL に置き換える。docs 例は `/v1/chat/completions`。`src/llm.ts` の `/v1/responses` も同じ置き換えを第一候補にする。Gateway が responses を拒むなら chat/completions に揃える
- 鍵: リクエストの `Authorization: Bearer` に xAI token、**BYOK**（Gateway にキーを預ける）、または **Unified Billing**（[Getting started](https://developers.cloudflare.com/ai-gateway/get-started/)、2026-08-07）
- キャッシュ: 同じ JSON の再生成（失敗リトライ）で効く。日をまたぐ号には効かない
- Workers AI（`@cf/...`）は grok-4.6 の置き場ではない。主経路にしない

| | xAI 直接 | Workers AI | AI Gateway → grok |
| --- | --- | --- | --- |
| モデル | `grok-4.6` | Cloudflare カタログ | **同じ grok-4.6** |
| 鍵 | `XAI_API_KEY` | アカウント課金 | xAI token / BYOK / Unified |
| 得るもの | 最短 | 別モデル | **ログ・キャッシュ・レート・障害の切り分け** |
| v1 | 使わない | 使わない | **採用** |

**採用: AI Gateway → grok。** 夜 0〜8 回でも、失敗を Cloudflare 側で見たい。直接呼び出しは Gateway 障害時の逃げ。Workers AI は主経路にしない。
...
| LLM が事実を発明 | High | 入力は D1 の JSON のみ。ツールなし。スキーマ外れはテンプレ。出典 URL は JSON にあるものだけ |
...
4. **ingested 未生成** は `generate_only`。**composed 未配信** は `deliver_only`。どちらも Beat からやり直さない。`deliverEmail` が `pipeline_runs` を終端にする。
...
7. `DELIVER=email`。問題なければ放置。Web は別 PR。

Feature flag は環境変数 `DELIVER=email|none`、`LLM_GENERATE=0|1`。最初の週は `DELIVER=none` かつ `LLM_GENERATE=0` でテンプレ号を D1 に書き、目で読んでから LLM を主にする。
...
3. **収集と生成は二段。** 収集は GitHub のみを D1 へ。生成は保存済み JSON のみ。主経路は AI Gateway → grok-4.6。失敗時テンプレ。事実の発明を禁止。LLM に GitHub ツールを付けない。
...
8. **LLM は AI Gateway → grok-4.6。** Workers AI は主にしない。xAI 直接は Gateway 障害時の逃げ。
...
| R10 | xAI / Gateway 障害 | Low | 同じ JSON でテンプレ。GitHub に戻らない |
...
- https://developers.cloudflare.com/ai-gateway/usage/providers/grok/ （2026-04-20）Grok プロキシ。`api.x.ai/v1` を `gateway.../grok` に置換
- https://developers.cloudflare.com/ai-gateway/get-started/ （2026-08-07）BYOK / Unified Billing / REST `/ai/v1/responses`
...
### PR 7 — 生成（AI Gateway → grok、失敗時テンプレ）

- **タイトル:** 保存済み JSON から AI Gateway 経由で号を書く
- **対象:** `src/llm.ts`（baseURL を Gateway）、`src/generate.ts`、`vars.LLM_GENERATE`、`generate_only`
- **依存:** PR 4（raw が D1 にあること）
- **内容:** GitHub に戻らない。print 対象の raw JSON だけ渡す。スキーマ外れ・発明は破棄してテンプレ。Gateway ログで失敗を見る。マイルストーン A のあとに載せる（まずテンプレ号を目で確認してから LLM を主にする）。

推奨マージ順: 1 → 2 → 3 → 4 → マイルストーン A（テンプレ号）→ 7（LLM 生成）→ 5（メール）。6 は独立。ドメインが無いときは 5 を飛ばして 6。
